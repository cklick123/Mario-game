...
def on_mouse_down(pos , button):
    global start , loding
    if icon.collidepoint(pos) and button == mouse.LEFT:
        start = True
        sounds.click.play()
        lodinf = True
...
def draw():
    ...
    if start == True:
        ...
        if gameover :
            ...
            text2 = screen.draw.text(f"{point}" , color = "white" , topleft = (690 , 332) , fontsize = 40 , fontname = "pixel")
            coin1.draw()
            mush.draw()
            coin1.x = 300
            coin1.y = 350 
